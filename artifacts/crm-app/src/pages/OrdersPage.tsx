...
const inputCls =
  "w-full px-3 py-2 rounded-lg border border-gray-200 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition bg-white";
...
export default function OrdersPage() {
  ...
  const fetchData = async () => {
    ...
    const [{ data: ord }, { data: prod }, { data: cust }] = await Promise.all([
      supabase
        .from("orders")
        .select(
          "*, order_items(*, product:products(id, name, code, stock, selling_price))",
        )
  ...
  const handleSave = async () => {
    ...
    for (const item of items) {
      if (!item.product_id) {
        setFormError("Select a product for each row.");
        return;
      }
      if (!item.quantity || parseInt(item.quantity) <= 0) {
        setFormError("Enter a valid quantity for each row.");
        return;
      }
    }
    setSaveLoading(true);
    ...
    if (orderErr || !order) {
      setFormError(orderErr?.message ?? "Failed to create order.");
  ...
  const handleStatusChange = async (order: Order, newStatus: string) => {
    ...
    if (statusErr || !data || data.length === 0) {
      setError(
        statusErr?.message ??
          "Status update failed. Check database permissions.",
      );
      setStatusLoading(null);
      return;
    }
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {orders.length} total orders · Stock deducted only on Shipped
          </p>
        </div>
        <button
          onClick={() => {
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50 text-left">
                {["Customer", "Phone", "Items", "Total", "Status", "Date"].map(
                  (h) => (
                    <th
                      key={h}
                ...
                <tr key={order.id} className="hover:bg-gray-50/70 transition">
                  ...
                  <td className="px-5 py-3.5 text-gray-500 text-xs">
                    {(order.order_items ?? []).map((i, idx) => {
                      const prod = Array.isArray(i.product)
                        ? i.product[0]
                        : i.product;
                      return (
                        <span
                  ...
                  <td className="px-5 py-3.5">
                    {statusLoading === order.id ? (
                      ...
                      <select
                        ...
                        className={`text-xs font-medium px-2.5 py-1 rounded-full border cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-400 transition ${statusColor(order.order_status)}`}
                      >
        ...
        >
          <div className="space-y-4">
            ...
            <div className="rounded-lg bg-blue-50 border border-blue-100 px-3 py-2 text-xs text-blue-600">
              Stock is <strong>not</strong> deducted when creating an order —
              only when status changes to <strong>Shipped</strong>.
            </div>
